...
namespace xar_engine::graphics::backend::unit::vulkan
{
    api::SwapChainReference IVulkanSwapChainUnit::make_swap_chain(const MakeSwapChainParameters& parameters)
    {
        ...
        const auto vk_format_list = get_state().vulkan_device.get_native_physical_device().get_vk_surface_format_khr_list(vulkan_window_surface->get_vulkan_surface().get_native());
        ...
        const auto vk_present_mode_list = get_state().vulkan_device.get_native_physical_device().get_vk_present_mode_khr_list(vulkan_window_surface->get_vulkan_surface().get_native());
            ...
            native::vulkan::VulkanSwapChain{
                {
                    ...
                    get_state().vulkan_graphics_queue,
    ...
    {
        const auto result = get_state().vulkan_resource_storage.get(parameters.swap_chain).begin_frame();
    ...
    {
        get_state().vulkan_resource_storage.get(parameters.command_buffer).begin(false);

        const auto& vulkan_command_buffer = get_state().vulkan_resource_storage.get(parameters.command_buffer);
        ...
        const VkImageMemoryBarrier image_memory_barrier_start{
            ...
            .image = get_state().vulkan_resource_storage.get(parameters.swap_chain).get_vk_image(parameters.image_index),
            .subresourceRange = {
        ...
        color_vk_rendering_attachment_info_khr.imageView = get_state().vulkan_resource_storage.get(parameters.color_image_view).get_native();
        ...
        color_vk_rendering_attachment_info_khr.resolveImageView = get_state().vulkan_resource_storage.get(parameters.swap_chain).get_vulkan_image_view(parameters.image_index).get_native();
        ...
        depth_vk_rendering_attachment_info_khr.imageView = get_state().vulkan_resource_storage.get(parameters.depth_image_view).get_native();
        ...
        vk_rendering_info_khr.renderArea = VkRect2D{
            VkOffset2D{}, get_state().vulkan_resource_storage.get(parameters.swap_chain).get_vk_extent()
        };
    ...
    void IVulkanSwapChainUnit::end_rendering(const EndRenderingParameters& parameters)
    {
        ...
        vkCmdEndRenderingKHR(get_state().vulkan_resource_storage.get(parameters.command_buffer).get_native());
    ...
    {
        const auto result = get_state().vulkan_resource_storage.get(parameters.swap_chain).end_frame(get_state().vulkan_resource_storage.get(parameters.command_buffer).get_native());
