...
{
    void IVulkanDeviceComponent::wait_idle()
    {
        get_state()._vulkan_device.wait_idle();
    }

    std::uint32_t IVulkanDeviceComponent::get_sample_count() const
    {
        return get_state()._vulkan_device.get_native_physical_device().get_vk_sample_count_flag_bits();
    }

    api::EFormat IVulkanDeviceComponent::find_depth_format() const
    {
